...
def is_empty_tree(tree):
    # Your code here
    return type(tree) == tuple and len(tree) ==0
...
# Time complexity of insert_tree: O(nlogn), where n == x
# For large values of n, there are log n number of recursive calls as the right side branch's number exponentially increases for larger values,
# resulting in the number of recursive calls exponentially decreasing thus approximately log n number of reursive calls.
# list and tuple concentation have a time complexity of O(n) and at every level of recursive call, a new object is being formed due to the build_tree function
# hence a time complexity of O(nlogn)
...
# Time complexity of contains: O(2**(logn)), where n == x
# the longest "branch" will be of length log n
# at each recursive call, it results in 2 more recursive calls (assuming both are valid), which means there are a total number of 2**log n number of recursive calls in the whole programme before it reaches the base case of a tree with only an entry and empty left and right branches (assuming x is that very last entry)
...
# Time complexity of flatten: O(n logn), where n is the number of elements in the tree, assuming n is very large
# the depth of the recursion tree is of length logn
# at each recursive call, a new list object is being created, thus a time complexity of O(n)
...
def sort_it(lst):
    return flatten(accumulate(lambda x, y: insert_tree(x,y), make_empty_tree(), lst))
...
# Time complexity of sort_it: O(n), where n is the len(lst)
# accumulate function has a time complexity of O(n) as there are n number of recursive calls
# insert_tree has a time complexity of O(nlogn)
# flatten has a time complexity of O(nlogn)
# at each recursive call in accumulate, insert_tree function is called thus the total time complexity for accumulate is O(n^2 logn)
# flatten is applied once, taking the tree as its input, thus time complexity is O((nlogn)* (n^2logn)) thus O(n**3logn
